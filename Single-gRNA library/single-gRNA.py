...
def extract_sequences(input_fastq_file, output_fasta_file, suffix, repeat_patterns):
    """
    ...
    suffix: Suffix string to append to the sequence description.
    repeat_patterns: A list containing regular expressions for the repeated sequences to search for.
    """
    with open(output_fasta_file, 'w', encoding='UTF-8') as file_out:
        ...
        for seq_record in SeqIO.parse(input_fastq_file, 'fastq'):
            ...
            if len(repeat_matches) == 2:
                ...
                fasta_header = ">" + seq_record.description + suffix + "\n"
                # Write the header and the extracted sequence to the file
                file_out.writelines([fasta_header, str(sequence_extract)])
...
def run_bowtie2(input_fasta_file, index_prefix, output_sam_file):
    """
    Execute the Bowtie2 alignment tool to align sequences from a FASTA file against a reference genome.

    Parameters:
    input_fasta_file (str): The path to the FASTA file containing sequences to align.
    index_prefix (str): The prefix for the Bowtie2 index files corresponding to the reference genome.
    output_sam_file (str): The path where the SAM file containing the alignment results will be saved.
    ...
    try:
        ...
        result = subprocess.run(['bowtie2', '-f', '-a', '-p', '10', '--local', '-x', index_prefix, '-U', input_fasta_file, '-S', output_sam_file, '-N', '1', '-L22'], check=True)
        # Check the return code to confirm success
        if result.returncode == 0:
            print(f"Bowtie2 alignment completed successfully. Output saved to '{output_sam_file}'.")
    except subprocess.CalledProcessError as e:
        # Print error message if Bowtie2 fails
        print(f"Error during Bowtie2 alignment for '{input_fasta_file}': {e}")
...
def process_sam(input_sam_file, output_mapping_file):
...
def process_fastq(input_fastq_file, index_prefix, suffix, repeat_patterns):
    """
    ...
    output_fasta_file = input_fastq_file.replace('.fq', '.fasta')
    output_sam_file = input_fastq_file.replace('.fq', '.sam')
    ...
    extract_sequences(input_fastq_file, output_fasta_file, suffix, repeat_patterns)
    # Run Bowtie2 alignment
    run_bowtie2(output_fasta_file, index_prefix, output_sam_file)
    # Process the generated SAM file
    process_sam(output_sam_file, processed_txt)
...
def gene_count_enrichment(input_gRNA_enrichment, output_gene_enrichment):
    """
...
def process_gene(input_gRNA_enrichment, parallel_name):
    """
    ...
    gene_count_enrichment(input_gRNA_enrichment, output_gene_enrichment)
...
def main():
    ...
    parser.add_argument("--suffix", default=" I", 
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        future_to_fastq = {
            executor.submit(process_fastq, fastq1, index_prefix, suffix, repeat_patterns): fastq1
        }
        if args.input_fastq2:
            future_to_fastq[executor.submit(process_fastq, fastq2, index_prefix, suffix, repeat_patterns)] = fastq2
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        future_to_gene = {
            executor.submit(process_gene, merged_output_file, parallel_name): merged_output_file
        }
