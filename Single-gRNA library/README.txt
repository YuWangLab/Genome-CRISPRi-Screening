...
The `single-gRNA.py` script handles the main data processing, while `single-gRNA.sh` is a Bash script to manage the overall workflow on an HPC cluster using SLURM and GNU Parallel.
...
├── single-gRNA.py             # Main Python script for analysis
├── single-gRNA.sh             # Bash script for running the pipeline
...
### Running the Pipeline
1. Modify the Bash script: Open `single-gRNA.sh` and set the parameters, such as:
...
### Output
- The analysis results will be saved in `/hpcfs/fhome/sunlet/single`, as specified in the `single-gRNA.sh` script.
